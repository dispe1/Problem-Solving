...
def partialSum(a):
    ...
    for i in range(1, len(a)):
        ret[i] = ret[i-1] + a[i]
    return ret
#어떤 배열의 부분합 psum[]이 주어질 대, 원래 배열의 a부터 b까지의 합을 구한다.
def rangeSum(psum, a, b):
    if a == 0: return psum[b]
    return psum[b] - psum[a-1]

#표준편차 = sqrt(분산)
#A[]의 제곱의 부분 합 벡터 sqpsum, A[]의 부분 합 벡터 psum이 주어질 때 A[a:b+1]의 분산을 반환한다.
...
#2차원 psum[y][x] = sum(arr[0:i+1][0:j+1])
...
#합이 0에 가까운 구간 -> psum 정렬 후 인접한 원소 확인
